...
describe('validator', () => {
  it('accepts a valid && query', () => {
    const q = parse('ext:ts && keyword:foo')
  ...
  it('rejects and() with one child', () => {
    ...
    expect(result.errors[0]?.message).toMatch(/&& requires at least 2/)
  })
...
describe('formatter', () => {
  it('formats a simple query', () => {
    ...
    expect(format(q)).toBe('ext:ts && keyword:checkout')
  })
  ...
  it('is idempotent (3 iterations)', () => {
    const dsl = 'ext:tsx,ts && keyword:api'
    const q1 = parse(dsl)
    const s1 = format(q1)
...
describe('linter', () => {
  ...
  it('reports contradictory-branches for !x and x in same &&', () => {
    const q = parse('ext:ts && !ext:ts')
    const result = lint(q)
  ...
  it('reports likely-zero-match for top-level !', () => {
    const q = parse('!ext:ts')
    const result = lint(q)
...
describe('CLI fmt subcommand', () => {
  it('formats a query to stdout', async () => {
    const result = await runCLI(['fmt', 'ext:tsx,ts && keyword:api'])
    expect(result.exitCode).toBe(0)
    expect(result.stdout.trim()).toBe('ext:ts,tsx && keyword:api')
  })
...
describe('CLI lint subcommand', () => {
  it('exits 0 with no issues for clean query', async () => {
    const result = await runCLI(['lint', 'ext:ts && keyword:foo'])
    expect(result.exitCode).toBe(0)
