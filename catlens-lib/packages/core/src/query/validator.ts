  ...
  switch (node.type) {
    ...
    case 'or':
      if (node.children.length < 2) {
        errors.push({ message: `${node.type}() requires at least 2 children, got ${node.children.length}` })
      }
...
function validatePredicate(pred: PredicateNode, errors: ValidationError[]): void {
  switch (pred.type) {
    ...
    case 'glob':
      if (!pred.pattern) {
        errors.push({ message: 'glob() requires a non-empty pattern' })
      }
      break
    case 'tag':
      if (!pred.tag) {
    ...
    case 'newer_than':
      if (pred.duration.value <= 0) {
        errors.push({ message: `${pred.type}() duration must be positive, got ${pred.duration.value}` })
      }
      break
    case 'diff':
