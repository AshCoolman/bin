...
describe('US1 — Ad Hoc Query and Render', () => {
  ...
  describe('boolean composition', () => {
    it('and: requires all children to match', async () => {
      const result = await runQuery(TS_APP, 'and(ext(ts,tsx), keyword("checkout"))')
      expect(result.files.length).toBeGreaterThan(0)
      expect(result.files.every(f => f.path.endsWith('.ts') || f.path.endsWith('.tsx'))).toBe(true)
      expect(result.files.every(f => f.content.toLowerCase().includes('checkout'))).toBe(true)
    })

    it('or: matches if any child matches', async () => {
      const andResult = await runQuery(TS_APP, 'and(ext(ts,tsx), keyword("checkout"))')
      const orResult = await runQuery(TS_APP, 'or(keyword("checkout"), keyword("api"))')
      expect(orResult.stats.fileCount).toBeGreaterThanOrEqual(andResult.stats.fileCount)
    })

    it('not: excludes matching files', async () => {
      const all = await runQuery(TS_APP, 'ext(ts)')
      const withKeyword = await runQuery(TS_APP, 'and(ext(ts), keyword("checkout"))')
      const withoutKeyword = await runQuery(TS_APP, 'and(ext(ts), not(keyword("checkout")))')
  ...
  describe('CLI --preview', () => {
    it('outputs file list summary', async () => {
      const r = await runCLI(['and(ext(ts,tsx), keyword("checkout"))', '--preview'], { root: TS_APP })
